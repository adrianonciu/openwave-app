...
Current backend infrastructure also includes a canonical Romanian county-based local source registry used by the source monitoring layer. When a listener region is available and local preference is enabled, SourceWatcher can operationally ingest region-first local media sources such as `ziaruldeiasi.ro`, `bzi.ro`, and `ieseanul.ro` for Iasi. City remains metadata only and does not activate local source monitoring.
