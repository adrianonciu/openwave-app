...
JSON_OUTPUT_V13_PATH = OUTPUT_DIR / "radio_editing_preview_v13.json"
TEXT_OUTPUT_V13_PATH = OUTPUT_DIR / "radio_editing_preview_v13.txt"
...
LIVE_NICU_CONSTANTA_V13_OUTPUT_PATH = OUTPUT_DIR / "live_generalist_bulletin_nicu_constanta_v13.txt"
...
def _render_live_preview_text(preview_payload: dict[str, object]) -> str:
    lines = [
        ...
        f"Stories generated by llm: {preview_payload.get('stories_generated_by_llm', 0)}",
        f"Stories skipped no actor: {preview_payload.get('stories_skipped_no_actor', 0)}",
        f"Stories failed llm parse: {preview_payload.get('stories_failed_llm_parse', 0)}",
        f"Stories using fallback rules: {preview_payload.get('stories_using_fallback_rules', 0)}",
    ...
    for item in preview_payload['stories']:
        lines.extend([
            ...
            f"   Summarization method: {item.get('summarization_method', '') or 'unknown'}",
            f"   Actor detected: {item.get('actor_detected', '') or 'unknown'}",
            f"   Quote detected: {item.get('quote_detected', '') or 'unknown'}",
            f"   Impact detected: {item.get('impact_detected', '') or 'unknown'}",
...
def _run_live_mode(user: str = "Nicu", county: str = "Constanta", save_sample_dir: str | None = None) -> None:
    ...
    JSON_OUTPUT_V13_PATH.write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    ...
    TEXT_OUTPUT_V13_PATH.write_text(preview_text, encoding="utf-8")
    ...
    LIVE_NICU_CONSTANTA_V13_OUTPUT_PATH.write_text(bulletin_text, encoding="utf-8")
